import { BangloanFixedYieldVaultFactory__factory, BangloanUpsideVaultFactory__factory } from '@bangloan/contracts';
...
export class NotificationsService implements OnModuleInit {
  ...
  async startListeningForBlockEvents() {
    ...
    let vaultInterface = new ethers.utils.Interface(BangloanFixedYieldVaultFactory__factory.abi);
          ...
          if (tx.logs.length > 0) {
            tx.logs.forEach(async (log) => {
              if (await this.isUpsideVault(log)) {
                ...
                vaultInterface = new ethers.utils.Interface(BangloanUpsideVaultFactory__factory.abi);
              } else if (await this.isFixedYieldVault(log)) {
                console.log('true fixed yield vault');
                vaultInterface = new ethers.utils.Interface(BangloanFixedYieldVaultFactory__factory.abi);
              } else {
                return;
              }
  ...
  async isUpsideVault(log: ethers.providers.Log) {
    const { data, error } = await this.supabaseAdmin
      ...
      .select()
      .eq('contract_name', 'BangloanUpsideVaultFactory')
      .single();
  ...
  async isFixedYieldVault(log: ethers.providers.Log) {
    const { data, error } = await this.supabaseAdmin
      ...
      .from('contracts_addresses')
      .select()
      .eq('contract_name', 'BangloanFixedYieldVaultFactory')
      .single();

import { BangloanFixedYieldVault, BangloanFixedYieldVault__factory } from '@bangloan/contracts';
...
export class MatureVaultsService {
  ...
  private async contract(vault: Tables<'vaults'>): Promise<BangloanFixedYieldVault> {
    return BangloanFixedYieldVault__factory.connect(vault.address, await this.ethers.operator());
  }

  private async strategy(vault: Tables<'vaults'>): Promise<BangloanFixedYieldVault> {
    return BangloanFixedYieldVault__factory.connect(vault.strategy_address, await this.ethers.operator());
  }
}

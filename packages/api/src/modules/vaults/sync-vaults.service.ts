...
import { VaultDeployedEvent } from '@bangloan/contracts/types/BangloanFixedYieldVaultFactory';
import { VaultDeployedEvent as UpsideVaultDeployedEvent } from '@bangloan/contracts/types/BangloanUpsideVaultFactory';
...
export class SyncVaultsService {
  ...
  private async getFactoryContract(addr: string): Promise<BangloanFixedYieldVaultFactory> {
    return BangloanFixedYieldVaultFactory__factory.connect(addr, await this.ethers.operator());
  }

  private async getFactoryUpsideContract(addr: string): Promise<BangloanUpsideVaultFactory> {

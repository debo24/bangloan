import { BangloanFixedYieldVault__factory } from '@bangloan/contracts';
...
export async function getFactoryContractAddress(
  ...
  return getContractAddress(chainId, 'BangloanFixedYieldVaultFactory', supabase);
}
...
export async function getFactoryUpsideContractAddress(
  ...
  return getContractAddress(chainId, 'BangloanUpsideVaultFactory', supabase);
}
  ...
  return _.compact(
    await Promise.all(
      (vaults.data || []).map(async (vault) => {
        const vaultContract = BangloanFixedYieldVault__factory.connect(vault.address, operator);

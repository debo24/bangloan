import {
  BangloanFixedYieldVault,
  BangloanFixedYieldVaultFactory,
  BangloanFixedYieldVaultFactory__factory,
  BangloanFixedYieldVault__factory,
  BangloanUpsideVaultFactory,
  BangloanUpsideVaultFactory__factory,
} from '@bangloan/contracts';
import { FixedYieldVault } from '@bangloan/contracts/types/BangloanFixedYieldVault';
import { MaturityVault } from '@bangloan/contracts/types/BangloanFixedYieldVault';
import { UpsideVault } from '@bangloan/contracts/types/BangloanFixedYieldVaultWithUpside';
...
export class VaultsService {
  ...
  private async factoryContract(addr: string): Promise<BangloanFixedYieldVaultFactory> {
    return BangloanFixedYieldVaultFactory__factory.connect(addr, await this.ethers.operator());
  }

  private async vaultContract(addr: string): Promise<BangloanFixedYieldVault> {
    return BangloanFixedYieldVault__factory.connect(addr, await this.ethers.operator());
  }

  private async factoryUpsideContract(addr: string): Promise<BangloanUpsideVaultFactory> {
    return BangloanUpsideVaultFactory__factory.connect(addr, await this.ethers.operator());
  }
}

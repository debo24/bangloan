...
abstract contract VaultFactory is AccessControl {
    ...
    constructor(address owner, address operator, address[] memory custodians) {
        if (owner == address(0)) {
            revert BangloanVaultFactory__InvalidOwnerAddress();
        }

        if (operator == address(0)) {
            revert BangloanVaultFactory__InvalidOperatorAddress();
        }
        ...
        for (uint256 i = 0; i < custodians.length; i++) {
            if (custodians[i] == address(0)) {
                revert BangloanVaultFactory__InvalidCustodianAddress();
            }
    ...
    modifier onlyAllowedCustodians(address _custodian) virtual {
        if (!allowedCustodians.contains(_custodian)) {
            revert BangloanVaultFactory__CustodianNotAllowed();
        }
        _;
    }

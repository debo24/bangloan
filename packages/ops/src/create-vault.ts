import { BangloanFixedYieldVault__factory, VaultFactory__factory } from '@bangloan/contracts';
...
export async function createVault(
  ...
  const expectedFactoryName = isUpside ? 'BangloanUpsideVaultFactory' : 'BangloanFixedYieldVaultFactory';
  ...
  const whiteListProvider = addresses.data.find((i: any) => i.contract_name === 'BangloanWhiteListProvider')?.address;
  ...
  if (isMatured) {
    const vault = BangloanFixedYieldVault__factory.connect(created.address, adminSigner);

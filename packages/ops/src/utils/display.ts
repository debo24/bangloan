import { BangloanFixedYieldVault, BangloanFixedYieldVaultWithUpside, ERC20 } from '@bangloan/contracts';
...
async function internalDescribeVault(vault: BangloanFixedYieldVault): Promise<string> {
  return `
...
export async function describeFYVault(vault: BangloanFixedYieldVault): Promise<string> {
  return `
  Type: Fixed Yield\
...
export async function describeFYWUVault(vault: BangloanFixedYieldVaultWithUpside): Promise<string> {
  return `
  Type: Fixed Yield With Upside\
  ${await internalDescribeVault(vault as BangloanFixedYieldVault)}
